...
class WorkshopJob(models.Model):
    ...
    # Customer & Vehicle
    customer_id = fields.Many2one('workshop.customer', string='Customer', required=True, tracking=True)
    vehicle_id = fields.Many2one('workshop.vehicle', string='Vehicle', required=True, tracking=True, domain="[('customer_id', '=', customer_id)]")
    ...
    # Service Lines
    labour_ids = fields.One2many('workshop.job.labour', 'job_id', string='Labour')
    parts_ids = fields.One2many('workshop.job.parts', 'job_id', string='Parts')
    ...
    def create(self, vals):
        if vals.get('job_number', 'New') == 'New':
            vals['job_number'] = self.env['ir.sequence'].next_by_code('workshop.workshop.job') or 'New'
