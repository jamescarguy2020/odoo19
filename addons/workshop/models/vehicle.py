...
class WorkshopVehicle(models.Model):
    ...
    # Owner
    customer_id = fields.Many2one('workshop.customer', string='Owner', required=True, tracking=True, ondelete='restrict')
    ...
    # Vehicle Details
    brand_id = fields.Many2one('workshop.vehicle.brand', string='Brand', tracking=True)
    ...
    # Relationships
    job_ids = fields.One2many('workshop.workshop.job', 'vehicle_id', string='Service Jobs')
    ...
    def _onchange_brand_name(self):
        ...
        if self.brand_name and not self.brand_id:
            brand = self.env['workshop.vehicle.brand'].find_brand_by_abbreviation(self.brand_name)
            if brand:
    ...
    def action_view_jobs(self):
        ...
        return {
            ...
            'res_model': 'workshop.workshop.job',
            'view_mode': 'tree,form',
