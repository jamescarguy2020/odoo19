        ...
        for match in matches:
            try:
                ...
                if len(values) >= 10:
                    ...
                    existing = self.env['pg.motors.customer'].search([('customer_id', '=', customer_id)], limit=1)
                    if not existing:
                        # Create customer
                        self.env['pg.motors.customer'].create({
        ...
        for match in matches:
            try:
                ...
                if len(values) >= 5:
                    ...
                    # Find customer
                    customer = self.env['pg.motors.customer'].search([('customer_id', '=', customer_id_ref)], limit=1)
                    if not customer:
                        log.append(f"Customer {customer_id_ref} not found for vehicle {registration}")
                        continue
                    
                    # Check if vehicle already exists
                    existing = self.env['pg.motors.vehicle'].search([('registration_number', '=', registration)], limit=1)
                    if not existing:
                        # Auto-match brand
                        brand = False
                        if self.auto_match_brands and brand_name:
                            brand = self.env['pg.motors.vehicle.brand'].find_brand_by_abbreviation(brand_name)
                            if brand:
                                brands_matched += 1
                        
                        # Create vehicle
                        self.env['pg.motors.vehicle'].create({
